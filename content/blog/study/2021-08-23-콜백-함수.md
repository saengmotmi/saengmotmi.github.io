...
클로저와 함께 사용하는 경우도 주요한 패턴 중 하나다. `Debounce`를 대표적인 예시로 들 수 있겠다.
...
>
> [<Throttle 와 Debounce 개념 정리하기>](https://pks2974.medium.com/throttle-%EC%99%80-debounce-%EA%B0%9C%EB%85%90-%EC%A0%95%EB%A6%AC%ED%95%98%EA%B8%B0-2335a9c426ff), 박성룡
...
`Promise` 등장 전까지는 비동기 처리를 callback으로만 처리했기 때문에 이전에 callback 패턴으로 작성된 비동기 함수들을 Promise화 하기도 한다.
...
// 함수를 받아서 일단 함수를 리턴
// 그렇게 리턴된 함수가 실행될 때 인자가 두 개 이상이라면 즉시 실행
// 아니라면 이후에 들어온 인자까지 합쳐서 그 때 실행
...
const reduce = curry((f, acc, iter) => {
  ...
  for (const a of iter) {
    acc = f(acc, a) // 연산 자체를 직접 써주지 않고 함수에 위임
  }
  return acc
})

const total_price = pipe(
  // pipe : 함수를 합성한 결과를 리턴하는 함수
  map(p => p.price),
  reduce(add)
)
