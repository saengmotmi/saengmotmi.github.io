...
const go = (...args) => {
  // reduce(f, [acc,] iter)
  // reduce는 iter.next()에서 넘겨받은 값을 f로 실행시킨 결과를 acc에 누적
  // 지금은 acc 초기값이 주어지지 않아 iter.next()의 첫번째 값이 acc의 초기값으로 설정됨
  reduce((a, f) => f(a), args)
}
...
// pipe : reduce의 응용, 함수를 합성한 결과를 리턴하는 함수
// as는 나중에 부분실행되면서 받을 초기값
